...
# ACDS 25.1 129 linux 2025.09.15.12:31:52
# ----------------------------------------
...
# ----------------------------------------
# If rspfifo is one of several IP cores in your
...
if ![info exists TOP_LEVEL_NAME] { 
  set TOP_LEVEL_NAME "rspfifo.rspfifo"
}
...
set LD_LIBRARY_PATH [dict merge $LD_LIBRARY_PATH [dict get [rspfifo::get_env_variables $SIMULATOR_TOOL_BITNESS] "LD_LIBRARY_PATH"]]
if {[dict size $LD_LIBRARY_PATH] !=0 } {
  set LD_LIBRARY_PATH [subst [join [dict keys $LD_LIBRARY_PATH] ":"]]
  setenv LD_LIBRARY_PATH "$LD_LIBRARY_PATH"
}
append ELAB_OPTIONS [subst [rspfifo::get_elab_options $SIMULATOR_TOOL_BITNESS]]
append SIM_OPTIONS [subst [rspfifo::get_sim_options $SIMULATOR_TOOL_BITNESS]]
...
alias file_copy {
  ...
  set memory_files [concat $memory_files [rspfifo::get_memory_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
  ...
  set normalized_src [rspfifo::normalize_path "$file"]
  set normalized_dest [rspfifo::normalize_path "$dest_file"]
...
set libraries [dict merge $libraries [rspfifo::get_dpi_libraries "$QSYS_SIMDIR"]]
...
set design_libraries [dict merge $design_libraries [rspfifo::get_design_libraries]]
...
alias com {
  ...
  set design_files [dict merge [rspfifo::get_common_design_files $USER_DEFINED_COMPILE_OPTIONS $USER_DEFINED_VERILOG_COMPILE_OPTIONS $USER_DEFINED_VHDL_COMPILE_OPTIONS "$QSYS_SIMDIR"]]
  set common_design_files [dict values $design_files]
  foreach file $common_design_files {
    eval $file
  }
  set design_files [list]
  set design_files [concat $design_files [rspfifo::get_design_files $USER_DEFINED_COMPILE_OPTIONS $USER_DEFINED_VERILOG_COMPILE_OPTIONS $USER_DEFINED_VHDL_COMPILE_OPTIONS "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
