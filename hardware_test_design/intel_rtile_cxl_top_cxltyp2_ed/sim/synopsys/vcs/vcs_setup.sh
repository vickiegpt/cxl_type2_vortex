...
TOP_LEVEL_NAME="intel_rtile_cxl_top_cxltyp2_ed"
QSYS_SIMDIR="./../../"
...
if [ -n "${QSYS_SIM_SCRIPT_VCS_OPTIONS_FILE}" ] && [ -f ${QSYS_SIM_SCRIPT_VCS_OPTIONS_FILE} ]; then
  echo "Sourcing ${QSYS_SIM_SCRIPT_VCS_OPTIONS_FILE}" 
  source ${QSYS_SIM_SCRIPT_VCS_OPTIONS_FILE}
  if [ $? -ne 0 ]; then
    echo "Error:: This file ${QSYS_SIM_SCRIPT_VCS_OPTIONS_FILE} has invalid expression/s" >&2
    exit $?
  fi
fi
...
source $QSYS_SIMDIR/common/vcs_files.tcl
set files [dict merge $files [intel_rtile_cxl_top_cxltyp2_ed::get_common_design_files "$QSYS_SIMDIR"]]
...
set QUARTUS_INSTALL_DIR [lindex $argv 2]
source $QSYS_SIMDIR/common/vcs_files.tcl
if {[catch {intel_rtile_cxl_top_cxltyp2_ed::get_design_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"} errorMsg]} {
 puts "Catch error: $errorMsg"
 exit 1 
}
set files [dict create]
set files [dict merge $files [intel_rtile_cxl_top_cxltyp2_ed::get_design_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
...
run_vcs="vcs -lca -timescale=1ps/1ps -sverilog +verilog2001ext+.v $ELAB_OPTIONS $USER_DEFINED_ELAB_OPTIONS \
  -v $QUARTUS_SIM_LIB_DIR/220model.v \
  -v $QUARTUS_SIM_LIB_DIR/sgate.v \
  -v $QUARTUS_SIM_LIB_DIR/altera_primitives.v \
  -v $QUARTUS_SIM_LIB_DIR/altera_mf.v \
  $QUARTUS_SIM_LIB_DIR/altera_lnsim.sv \
  $QUARTUS_SIM_LIB_DIR/tennm_atoms.sv \
  $QUARTUS_SIM_LIB_DIR/synopsys/tennm_atoms_ncrypt.sv \
  $QUARTUS_SIM_LIB_DIR/fmica_atoms_ncrypt.sv \
  $QUARTUS_SIM_LIB_DIR/tennm_hssi_atoms.sv \
  $QUARTUS_SIM_LIB_DIR/tennm_hssi_atoms_ncrypt.sv \
  $DEVICES_SIM_LIB_DIR/tennm_fm_hps.sv \
  $DEVICES_SIM_LIB_DIR/tennm_fm_hps_ncrypt.sv \
  $QUARTUS_SIM_LIB_DIR/ctfb_hssi_atoms.sv \
  $QUARTUS_SIM_LIB_DIR/ctfb_hssi_atoms_ncrypt.sv \
  $QUARTUS_SIM_LIB_DIR/synopsys/ctfb_hssi_atoms2_ncrypt.sv \
  $QUARTUS_SIM_LIB_DIR/ctr_hssi_atoms.sv \
  $QUARTUS_SIM_LIB_DIR/ctr_hssi_atoms_ncrypt.sv \
  $QUARTUS_SIM_LIB_DIR/ctrb_hssi_atoms_ncrypt.sv \
  $common_design_files \
  $design_files \
  $USER_DEFINED_ELAB_OPTIONS_APPEND \
  -top $TOP_LEVEL_NAME
