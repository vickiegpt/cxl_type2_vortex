...
module avmm_ic_altera_merlin_width_adapter_1950_m3lulhi 
#(
    ...
    //AXI5Lite Parameters
    parameter IN_PKT_AWAKEUP                = 114,
    parameter IN_PKT_ATRACE                 = 95,
    parameter IN_PKT_TRACE                  = 95,    
    ...
    parameter OUT_PKT_AWAKEUP               = 71,    
    parameter OUT_PKT_ATRACE                = 71,
    parameter OUT_PKT_TRACE                 = 71,     
...
( 
    ...
    always @* begin
       ...
       if (ROLE_BASED_USER)begin
           ...
           in_user_data_field        = '0;
           in_addrchk_field          = '0;     
       end       
       if(ENABLE_AXI5)begin
          in_awakeup_field = in_data[IN_PKT_AWAKEUP];
          in_atrace_field = in_data[IN_PKT_ATRACE];
          in_trace_field  = in_data[IN_PKT_TRACE];
        end
       else begin
           in_awakeup_field       = '0;
           in_atrace_field        = '0;
    ...
    generate
               ...
               if(~use_reg) begin
                  ...
                  if(ROLE_BASED_USER || ENABLE_AXI5 ) begin
                      datachk_reg <= datachk_reg >> OUT_DATACHK_W ;
                       if(ROLE_BASED_USER) begin
		                  user_data_reg     <= user_data_reg    >> (OUT_NUMSYMBOLS * USER_DATA_SYMBOL_W);
		       end
                  end
                  else  begin
                  datachk_reg <= '0;
         ...
         always @* begin
            ...
            if (CONSTANT_BURST_SIZE) begin // For Avalon only
                ...
                if(ROLE_BASED_USER || ENABLE_AXI5) begin
                   ...
                   if(ROLE_BASED_USER) begin
		                out_user_data_field     = in_user_data_field[OUT_NUMSYMBOLS * USER_DATA_SYMBOL_W-1:0];
		   end 
                end
                else begin
                   out_datachk_field  = '0;
            ...
            if (!CONSTANT_BURST_SIZE) begin
                ...
                if(ROLE_BASED_USER || ENABLE_AXI5) begin
                    ...
                    if(ROLE_BASED_USER) begin
		                 out_user_data_field = user_data_array[output_sel];
		     end
                end
                else begin 
                    out_datachk_field   = '0;
            ...
            if (OPTIMIZE_FOR_RSP | single_response_expected) begin
                ...
                for (i = 0; i < RATIO; i=i+1) begin
                    ...
                    if(ROLE_BASED_USER) begin
                            out_user_data_field |= user_data_mask & in_user_data_field[i*OUT_USER_DATA_SEGMENT_W +: OUT_USER_DATA_SEGMENT_W];
                    end
                    end
                    else begin
                       out_user_data_field = '0;
         ...
         if (!CONSTANT_BURST_SIZE) begin
             ...
             always @* begin
                 for(ibyte=0; ibyte<RATIO; ibyte=ibyte+1) begin: mux_mapping
                     ...
                     if(ROLE_BASED_USER) begin
                             user_data_array[ibyte] = in_user_data_field[(ibyte*OUT_NUMSYMBOLS*USER_DATA_SYMBOL_W)+:OUT_NUMSYMBOLS*USER_DATA_SYMBOL_W];
                      end
                     end
                     else begin
      ...
      if (OUT_NUMSYMBOLS > IN_NUMSYMBOLS)  begin
        ...
        always @* begin
            ...
            if(BITSPERBYTE && ROLE_BASED_USER)
            ...
            if (p1_cmpr_read && (p1_byte_cnt_field > IN_NUMSYMBOLS)) begin
                ...
                if (PACKING == 1) begin  // byteenable only affect on command path
                    ...
                    if (ROLE_BASED_USER|| ENABLE_AXI5) begin
                        ...
                        out_datachk_field = '0;
                        out_datachk_field[p1_shift_correct_ouput_segments*IN_NUMSYMBOLS +: IN_NUMSYMBOLS] = '0; 
                    end
   ...
   always @* begin
      ...
      if (ENABLE_AXI5)begin
          out_data[OUT_PKT_AWAKEUP] = in_awakeup_field;
          out_data[OUT_PKT_ATRACE] = in_atrace_field;
          out_data[OUT_PKT_TRACE ] = in_trace_field;
      end	      
   end // always @ *
