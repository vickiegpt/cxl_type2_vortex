...
set pld_clk      [get_clocks $ip_inst_name|inst_cxl_type2_top|cxl_ip|rtile_cxl_ip.coreclkout_hip]
set side_hip_clk [get_clocks $ip_inst_name|inst_cxl_type2_top|cxl_ip|rtile_cxl_ip.pld_clkout_slow]
set cam_clk      [get_clocks $ip_inst_name|inst_cxl_type2_top|cxl_t2ip_sip_top|gen_clkrst|cxl_memexp_sip_clkgen|rnr_ial_sip_clkgen_pll_cam_clk]
set sbr_clk      [get_clocks $ip_inst_name|inst_cxl_type2_top|cxl_t2ip_sip_top|gen_clkrst|cxl_memexp_sip_clkgen|rnr_ial_sip_clkgen_pll_sbr_clk]
set tck          [get_clocks altera_reserved_tck]
#set emif_usr_clk_0 [get_clocks cxltyp3ddr_0|cxltyp3ddr_inst|cxltyp3ddr_sub|cxl_mc_top|emif_inst_0_core_usr_clk]
#set emif_usr_clk_1 [get_clocks cxltyp3ddr_0|cxltyp3ddr_inst|cxltyp3ddr_sub|cxl_mc_top|emif_inst_1_core_usr_clk]

#-----------------------------------------------
# Set Exceptions
#-----------------------------------------------
set_false_path -to $ip_inst_name|inst_cxl_type2_top|cxl_t2ip_sip_top|gen_clkrst|*_rstn_pipe*|clrn
set_false_path -to $ip_inst_name|inst_cxl_type2_top|cxl_t2ip_sip_top|gen_clkrst|*_rstn_pipe[0]|d

#-----------------------------------------------
# Set Clock groups
#-----------------------------------------------

set_clock_groups -asynchronous -group $pld_clk -group $cam_clk -group $sbr_clk -group $side_hip_clk -group $tck
...
#set sync_path expr{[$hier_path|SYNC] == 0}

#set sync_path [get_pins $hier_path|SYNC]

puts "Hier path is $hier_path"

if {[string match {*tlp_crd_chk*} $hier_path] || [string match {*qs_fifo*} $hier_path] || [string match {*pq_cfg_read_fifo*} $hier_path] || [string match {*ip2user*aer_fifo*} $hier_path]} {
    puts "Hier path in1 is $hier_path"
    set sync_path 0
} else {
    puts "Hier path in2 is $hier_path"
    set sync_path 1
}

puts "Sync path is $sync_path"
...
if {$sync_path} {
    ...
    puts "Hier path inside is $hier_path"
    }
}
#
...
apply_sdc_pre_dcfifo cxl_memexp_sip_io_shim_fifo_vcd*
#apply_sdc_pre_dcfifo cxl_memexp_sip_bbs_fifo_vcd*
#apply_sdc_pre_dcfifo ccv_afu_cdc_fifo_vcd*
apply_sdc_pre_dcfifo cxl_io_fifo_vcd*
#-----------------------------------------------
